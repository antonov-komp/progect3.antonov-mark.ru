<?php
/**
 * Проверка: какой процессор используется через веб-сервер
 */

// Подключение из публичной директории
// Путь: public/outgoing-webhook/tools/ -> outgoing-webhook/
// dirname(__DIR__, 3) = /var/www/progect3.antonov-mark.ru
$basePath = dirname(__DIR__, 3);
require_once $basePath . '/outgoing-webhook/bootstrap.php';
require_once $basePath . '/outgoing-webhook/services/bootstrap.php';
        ...
        try {
            if ($hasEventRepo && $hasQueueRepo) {
                ...
                if ($eventRepo !== null && $queueRepo !== null) {
                    ...
                    $processorType = 'Репозитории null (БД недоступна) ✗';
                }
            } else {
