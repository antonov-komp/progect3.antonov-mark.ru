...
## Отметка о проделанной работе
**Дата:** 2026-01-23 20:15 (UTC+03:00, Брест)  
**Статус:** В работе  
**Сделано:**
- Созданы сервисы в `outgoing-webhook/services/` (Config/Rest/Queue/Enrichment/Dicts/Logging/Task).
- Реализован `QueueRunner` и вынесена orchestration очереди.
- `process-queue.php` переведён на тонкую обёртку с shim‑функциями.
- Добавлена CLI‑обёртка `outgoing-webhook/tools/process-queue-cli.php`.
- Обновлены документы `DOCS/OUTGOING-EVENTS/README.md`, `bootstrap/README.md`, `01-how-it-works.md`.
