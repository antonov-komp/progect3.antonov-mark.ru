...
- [x] Вход в модуль: плитка «Пользовательские поля» на главной странице ведёт на `/user-fields`
- [x] Страница навигации разделов отображает: Сделки, Лиды, Контакты, Компании, смарт-процессы (подгружаются сразу при открытии)
- [x] При клике на раздел загружаются все поля раздела и отображаются в таблице
- [x] Таблица полей содержит колонки: Название пользовательское, ID технический, Тип поля, ID записи, Обязательное, Множественное, Сортировка, В списке, Редактирование в списке, В фильтре, Поиск
- [x] UserFieldService покрывает все 5 категорий (Deal, Lead, Contact, Company, Smart)
- [x] API возвращает корректный JSON для `section=sections` и `section=deal|lead|contact|company|smart`
- [x] Ошибки API логируются, UI показывает fallback при сбое
- [x] Доступ контролируется через AccessControlService
...
- 2026-02-12 (UTC+3, Брест): Критерии приёмки отмечены как выполненные по плану DOCS/PLAN/TASK-034-035-036-action-plan.md.