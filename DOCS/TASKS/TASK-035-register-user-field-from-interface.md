...
- [x] На странице списка полей раздела отображается кнопка «Создать поле»
- [x] По клику открывается модальное окно с формой создания
- [x] Форма содержит: название, код поля (опц.), тип, обязательное, множество, в списке, редактировать в списке, в фильтре, сортировка
- [x] При отправке формы выполняется POST к API, вызывается соответствующий метод Bitrix24
- [x] При успехе: модалка закрывается, список полей обновляется, показывается уведомление
- [x] При ошибке: отображается сообщение об ошибке
- [x] Поддержаны все разделы: Сделки, Лиды, Контакты, Компании, Смарт-процессы
- [x] Новое поле появляется в Bitrix24 в стандартной форме карточки элемента (не placement)
- [x] Поле является кастомным (UF_CRM_*), а не стандартным встроенным полем B24
...
- 2026-02-12 (UTC+3, Брест): Добавлен чекбокс EDIT_IN_LIST в форму создания; критерии приёмки отмечены как выполненные; статус обновлён на «Завершена» (план DOCS/PLAN/TASK-034-035-036-action-plan.md).