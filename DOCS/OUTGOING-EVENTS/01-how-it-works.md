...
3. Фоновый обработчик очереди (`tools/process-queue.php` или CLI `tools/process-queue-cli.php`)
   обогащает событие через REST API.
...
Файлы: `outgoing-webhook/tools/process-queue.php` и `outgoing-webhook/tools/process-queue-cli.php`
...
- Исполнение обёрнуто в сервисы `outgoing-webhook/services/` (Queue/Enrichment/Task/Logging).
...
- 2026-01-23 20:10 (UTC+03:00, Брест): отражён CLI‑запуск и сервисы очереди.
