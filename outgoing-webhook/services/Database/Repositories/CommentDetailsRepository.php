...
/**
 * Репозиторий для работы с деталями комментариев в БД
 * 
 * Ответственность:
 * - Сохранение деталей комментариев
 * - Чтение деталей комментариев
 * - Поиск по различным критериям
 */
class CommentDetailsRepository
{
    ...
    /**
     * Создать запись деталей комментария
     * 
    ...
    {
        $sql = "INSERT INTO comment_details (
            request_id, event_type, task_id, comment_id, details, formatted_details, created_at
        ) VALUES (
            :request_id, :event_type, :task_id, :comment_id, :details, :formatted_details, :created_at
        )";
        ...
        $params = [
            ...
            ':comment_id' => $detailsData['commentId'] ?? null,
        ...
        try {
            ...
            $this->errors->log('Failed to create comment details', [
                'request_id' => $detailsData['requestId'] ?? null,
                'task_id' => $detailsData['taskId'] ?? null,
                'comment_id' => $detailsData['commentId'] ?? null,
                'error' => $e->getMessage(),
    ...
    /**
     * Найти детали комментария по request_id
     * 
    ...
    {
        $sql = "SELECT * FROM comment_details WHERE request_id = :request_id LIMIT 1";
    ...
    /**
     * Найти детали комментария по task_id
     * 
    ...
    {
        $sql = "SELECT * FROM comment_details 
    ...
    /**
     * Найти детали комментария по comment_id
     * 
    ...
    {
        $sql = "SELECT * FROM comment_details WHERE comment_id = :comment_id LIMIT 1";
        $result = $this->database->queryOne($sql, [':comment_id' => $commentId]);
    ...
    /**
     * Найти детали комментария по типу события
     * 
